...
import planeIconCollectedUpgradable from '../assets/plane_icon_collected_upgradable.png'
...
export default function Map(props:any) {
  ...
  */
  ...
  const getPlaneIcon = (plane:Plane) => {
    ...
    const tmpNewCard = new Card(plane, loggedUsername, "tmpUrl")
    if(plane.onGround && !planeInUserCards){
      ...
      if(getUserCardByIcao24(plane.icao24).cardQuality < tmpNewCard.cardQuality){
        return planeIconCollectedUpgradable
      } else{
  ...
  function getUserCardByIcao24(icao24:String){

    const cardIds = Object.keys(userCards)
      for (let i = 0; i < cardIds.length; i++){
        if(userCards[cardIds[i]].planeIcao24 == icao24){
          return userCards[cardIds[i]]
        }
      }
      return false
  }
