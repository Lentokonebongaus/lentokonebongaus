...
import { Button as KittenButton, Icon, Spinner } from '@ui-kitten/components';
...
export default function PlayView(props: any) {
    ...
    const renderCardsListLoading = () => {
        return (
            <View style={styles.playViewCardsLoading}>
                {renderSpinner()}
    ...
    const renderDataTableRow = (card: Card, index: number) => {
        if (userCards.includes(card.planeIcao24) == false) {
            return (
                <DataTable.Row style={currentPlayerPlanesData[index].distance == undefined ? styles.playtableRowCardUnavailable : undefined} onPress={() => { handleCardPick(index) }}>
                    <DataTable.Cell style={styles.playDataTableCell}>{currentPlayerPlanesData[index].velocity ? parseInt(currentPlayerPlanesData[index].velocity) : null} <Text>{currentPlayerPlanesData[index].velocity != undefined ? "m/s" : null}</Text></DataTable.Cell>
                    <DataTable.Cell style={styles.playDataTableCell}>{currentPlayerPlanesData[index].geometricAltitude ? parseInt(currentPlayerPlanesData[index].geometricAltitude) : null}<Text>{currentPlayerPlanesData[index].geometricAltitude != undefined ? " km" : null}</Text></DataTable.Cell>
                    <DataTable.Cell style={styles.playDataTableCell}>{currentPlayerPlanesData[index].distance ? parseInt(currentPlayerPlanesData[index].distance) : null}<Text>{currentPlayerPlanesData[index].distance != undefined ? " km" : null}</Text></DataTable.Cell>
                    <DataTable.Cell style={styles.playDataTableCell}>{card.cardQuality}</DataTable.Cell>
                    <DataTable.Cell style={styles.playDataTableCell}><Text style={currentPlayerPlanesData[index].distance == undefined ? undefined : { color: "lime" }}>{currentPlayerPlanesData[index].distance == undefined ? "No" : "Yes"}</Text></DataTable.Cell>
                    <DataTable.Cell style={styles.playDataTableCell}>{card.planeManufacturer}</DataTable.Cell>
                    <DataTable.Cell style={styles.playDataTableCell}>{card.planeModel}</DataTable.Cell>
                    <DataTable.Cell style={styles.playDataTableCell}>{card.planeOwner ? card.planeOwner : card.planeOperator}</DataTable.Cell>
                </DataTable.Row>
            )
        }
    }
                ...
                <NativeCard containerStyle={{ backgroundColor: "#333C83", paddingHorizontal: 20, height: "80%" }} wrapperStyle={{ backgroundColor: "#333C83" }}>
                    <View style={{ display: "flex", flexDirection: "row" }}>
                        ...
                        <View style={{ position: "absolute", top: 30, right: 10 }}>
                            <Text style={styles.cardTextSmall}>Card wins: {currentComputerCard.wins}</Text>
                            <Text style={styles.cardTextSmall}>Card losses: {currentComputerCard.losses}</Text>
                        </View>
                    </View>
                    <NativeCard.Divider />
                    <View style={{ display: "flex", flexDirection: "row" }}>
                        <View style={{ marginRight: 10, maxWidth: "50%" }}>
                            <Text style={styles.cardTextSmall}>{currentComputerCard.planeManufacturer}</Text>
                            <Text style={styles.cardTextSmall}>{currentComputerCard.planeModel}</Text>
                            <Text style={styles.cardTextSmall}>{currentComputerCard.planeOwner}</Text>
                            <Text style={styles.cardTextSmall}>Card owner: {currentComputerCard.cardOwner}</Text>
                        </View>
                        <View style={{ maxWidth: "50%" }}>
                            <Text style={styles.cardTextSmall}>Card quality: {currentComputerCard.cardQuality}</Text>
                            <Text style={styles.cardTextSmall}>Current altitude: {currentComputerCardApiData.geometricAltitude ? Math.floor(currentComputerCardApiData.geometricAltitude) : null} km</Text>
                            <Text style={styles.cardTextSmall}>Current velocity: {currentComputerCardApiData.velocity ? Math.floor(currentComputerCardApiData.velocity) : null} m/s</Text>
                            <Text style={styles.cardTextSmall}>Distance: {Math.floor(currentComputerCardApiData.distance)} km</Text>
                        </View>
                    </View>
                </NativeCard>
            )
            ...
            return (
                <NativeCard containerStyle={{ backgroundColor: "#333C83", paddingHorizontal: 20, height: "80%" }} wrapperStyle={{ backgroundColor: "#333C83" }}>
                    <View style={{ display: "flex", flexDirection: "row" }}>
                        ...
                        <View style={{ position: "absolute", top: 30, right: 10 }}>
                            <Text style={styles.cardTextSmall}>Card wins: {currentPlayerCard.wins}</Text>
                            <Text style={styles.cardTextSmall}>Card losses: {currentPlayerCard.losses}</Text>
                        </View>
                    </View>
                    <NativeCard.Divider />
                    <View style={{ display: "flex", flexDirection: "row" }}>
                        <View style={{ marginRight: 10, maxWidth: "50%" }}>
                            <Text style={styles.cardTextSmall}>{currentPlayerCard.planeManufacturer}</Text>
                            <Text style={styles.cardTextSmall}>{currentPlayerCard.planeModel}</Text>
                            <Text style={styles.cardTextSmall}>{currentPlayerCard.planeOwner}</Text>
                        </View>
                        <View style={{ maxWidth: "50%" }}>
                            <Text style={styles.cardTextSmall}>Card quality: {currentPlayerCard.cardQuality}</Text>
                            <Text style={styles.cardTextSmall}>Current altitude: {currentPlayerCardApiData.geometricAltitude ? Math.floor(currentPlayerCardApiData.geometricAltitude) : null} km</Text>
                            <Text style={styles.cardTextSmall}>Current velocity: {currentPlayerCardApiData.velocity ? Math.floor(currentPlayerCardApiData.velocity) : null} m/s</Text>
                            <Text style={styles.cardTextSmall}>Distance: {Math.floor(currentPlayerCardApiData.distance)} km</Text>
                        </View>
                    </View>
                </NativeCard>
            )
    ...
    return (
        <View style={{ flex: 1 }}>
            <View style={styles.playCardsArea}>
